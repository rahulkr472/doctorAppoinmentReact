    ...
    <div className=" md:w-[100%] p-5">
      <div className="card md:flex items-center md:space-x-8 bg-white p-6 rounded-lg shadow-lg ">
        ...
        <div className="text-[16px] sm:mt-8">
          ...
          <div className="mt-3 space-y-1">
            ...
            <p className="text-sm text-gray-500">
              ...
              {["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]
                .filter((day) => userPersonalInfo?.workingDays?.[day])
                .map((day) => day.charAt(0).toUpperCase() + day.slice(1))
                .join(", ")}
            </p>
            <p className="text-sm text-gray-500">Start Time: {userPersonalInfo?.workStartTime} AM</p>
            <p className="text-sm text-gray-500">End Time: {userPersonalInfo?.workEndTime} PM</p>
            <p className="text-sm text-gray-500">
              Off Day: {new Date(userPersonalInfo?.timeOff).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
            </p>
          ...
          <div className="mt-4">
            <span
              className={`px-3 py-1 rounded-full text-white text-xs ${userPersonalInfo.status === "active"
                ? "bg-green-500"
                : "bg-gray-500"
                }`}
            >
